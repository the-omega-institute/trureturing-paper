...
M.spec = {
  consumes = { "paper_intuition_input_seen" },
...
function pipeline(event)
  ...
  if not root then error("register-research-intuition: " .. tostring(err)) end
  local envelope = research.read_envelope(
    envelope_path,
    "paper-intuition-input-envelope.v1")
  ...
  local result = research.run(paths, {
    ...
    "--topology-receipt", research.required(envelope.topology_receipt_path, "topology_receipt_path"),
    "--intuition-release", research.required(envelope.intuition_release_path, "intuition_release_path"),
    "--cursor", paths.work .. "/intuition-cursor.v1.json",
  })
