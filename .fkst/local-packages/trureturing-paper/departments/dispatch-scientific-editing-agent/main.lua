...
M.spec = {
  consumes = { "paper_scientific_manuscript_ready" },
...
function pipeline(event)
  ...
  if payload.next_route ~= "scientific-editing"
      ...
      or payload.paper_id == ""
      or type(payload.manuscript) ~= "table"
      or payload.manuscript.schema ~= "paper-scientific-manuscript.v1"
      or not agent.is_sha256(payload.manuscript.artifact_ref) then
    error("dispatch-scientific-editing-agent: exact admitted manuscript identity is required")
  end
  ...
  local staged = research.run(paths, {
    ...
    "--source-authoring-task-ref", payload.task_ref,
  }, paths.agent_cli)
  if type(staged) ~= "table"
      or staged.schema ~= "paper-scientific-editing-agent-task-staged.v1"
      or not agent.is_sha256(staged.dispatch_ref)
      or not agent.is_sha256(staged.task_ref)
      or staged.source_authoring_task_ref ~= payload.task_ref
      or staged.source_manuscript_ref ~= payload.manuscript.artifact_ref
      or staged.paper_id ~= payload.paper_id
      or staged.theory_program_ref ~= payload.theory_program_ref
      or staged.phase ~= "scientific-editing"
      or staged.agent_role ~= "paper-scientific-editor"
      or staged.context_mode ~= "claim-preserving-edit" then
    error("dispatch-scientific-editing-agent: Agent CLI returned an invalid staged task")
  end
  ...
  if type(registered) ~= "table"
    ...
    error("dispatch-scientific-editing-agent: registration changed staged task identity")
  end
  ...
  raise("paper_agent_task_requested", {
    ...
    scientific_editing_dispatch_ref = staged.dispatch_ref,
    source_authoring_task_ref = staged.source_authoring_task_ref,
    source_manuscript_ref = staged.source_manuscript_ref,
