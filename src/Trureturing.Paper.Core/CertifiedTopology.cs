...
public static class CertifiedTopologyReader
{
        ...
        JsonElement value)
    {
        ...
        foreach (JsonElement item in referencesValue.EnumerateArray())
        {
            ...
            Require(names.Contains("existing_node_id"),
                $"{itemPath} is missing required property 'existing_node_id'.");
            string? unknown = names.Except(
                ["existing_node_id", "missing_dependency_id", "missing_dependent_id"],
                StringComparer.Ordinal).FirstOrDefault();
            Require(unknown is null,
                $"{itemPath} contains unknown property '{unknown}'.");
            bool hasMissingDependency = names.Contains("missing_dependency_id");
            bool hasMissingDependent = names.Contains("missing_dependent_id");
            Require(hasMissingDependency != hasMissingDependent,
                $"{itemPath} must identify exactly one missing endpoint.");
            references.Add(new TopologyDanglingReference(
                ReadNonEmptyString(item, "existing_node_id", itemPath),
                hasMissingDependency
                    ? ReadNonEmptyString(item, "missing_dependency_id", itemPath)
                    : null,
                hasMissingDependent
                    ? ReadNonEmptyString(item, "missing_dependent_id", itemPath)
                    : null));
            index++;
        }
